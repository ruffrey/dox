...
exports.parseComments = function(js, options){
  ...
  for (var i = 0, len = js.length; i < len; ++i) {
    // start comment
    if (!withinMultiline && !withinSingle &&
        '/' == js[i] && '*' == js[i+1] && (!skipSingleStar || js[i+2] == '*'))
    {
      ...
      if (buf.trim().length) {
        ...
        if(comment) {
          ...
          comment.ctx = exports.parseCodeContext(code, comment);
        }
        buf = '';
      }
      i += 2;
...
exports.parseComment = function(str, options) {
  ...
  if (tags.length) {
    ...
    comment.tags = comment.tags.filter(function(tag){
      if (tag.type === 'fires') {
          comment.fires.push(tag.string);
          return false;
      } else if (tag.type === 'return' || tag.type === 'returns') {
          comment.returns = tag.string;
          return false;
      }
      return true;
    });
...
exports.parseTag = function(str) {
  ...
  var parts = lines[0].split(/ +/)
    , type = tag.type = parts.shift().replace('@', '').replace('!','');
...
exports.parseCodeContext = function(str, thisComment){
  var str = str.split('\n')[0];
  ...
  // object property with string key
  } else if (/^([\w$]+)\.([\w$]+)(\[.+\])[ \t]*=[ \t]*([^\n;]+)/.exec(str)) {
    return {
        type: 'property'
      , receiver: RegExp.$1
  ...
  //
    ...
    var eventString = "";
    var isEvent = thisComment.tags.some(function(tag){
      var typeIsEvent = tag.type == 'event';
      if(typeIsEvent) eventString = tag.string;
      return typeIsEvent;
    });
